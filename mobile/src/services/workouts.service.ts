...
export const createWorkoutBySessionService = async (
  sessionId: string,
  name: string,
): Promise<string> => {
  ...
  try {
    ...
    throw new Error(
      error instanceof Error
        ? error.message
        : "Failed to create workout by session",
    );
  }
};
...
export const updateWorkoutBySessionService = async (
  sessionId: string,
): Promise<string> => {
  try {
    ...
    throw new Error(
      error instanceof Error
        ? error.message
        : "Failed to update workout by session",
    );
  }
};
...
export const checkWorkoutNameExistsService = async (
  name: string,
): Promise<boolean> => {
  try {
